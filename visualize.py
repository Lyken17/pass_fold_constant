...
class MyVisistor(ExprVisitor):
    ...
    def visit_var(self, var):
        ...
        if self.display_vars:
            self.dot.node(name, 
                ...
                var.name_hint, 
            fillcolor="#EEEEEE", style="filled", fontsize="16")
    
    def visit_tuple(self, tup):
        res =  super().visit_tuple(tup)
        return res
    ...
    def visit_call(self, call):
        ...
        self.dot.node(f"{name}-{count}", mfstr, shape="box3d", 
                        fontsize="16",
                        style="filled", fillcolor="#335588", fontcolor="#FFFFFF", color="#000000")
        ...
        for idx, a in enumerate(call.args):
            ...
            if isinstance(a, relay.expr.Call):
                ...
                if idx != 0 :
                    w = 2
                self.dot.edge(self.id2name[a.handle.value], f"{name}-{count}", weight=str(w),
                    splines="polyline",
                )
...
if __name__ == "__main__":
    ...
    ir_path = "ir_zoos/fwd-1x3x128x128.ir"
    if len(sys.argv) >= 2:
        ir_path = sys.argv[-1]
